 ...
 * - Enhanced error reporting (catches specific PHP errors via Throwable).
 * - Implemented unlimited execution time `set_time_limit(0)`.
 * - Added strict checks for module existence before calling.
 * - detailed logging for success/failure states.
 *
 * @package   ReadyStudio
 * @version   15.0.0
 * @author    Fazel Ghaemi
...
class ReadyStudio_Core_Bulk {
	...
	*/
						...
						<?php while ( $query->have_posts() ) : $query->the_post(); ?>
							...
							<tr id="post-<?php echo get_the_ID(); ?>">
								...
								<td class="column-status" id="status-<?php echo get_the_ID(); ?>">
									<?php if ( $has_seo ) : ?>
										<span class="status-badge status-done">انجام شده</span>
									<?php else : ?>
	...
	public function ajax_handle_bulk_generate() {
		...
		if ( empty( self::$api ) ) {
			wp_send_json_error( 'API Error: AI Brain not initialized.' );
			return;
		}
		...
		try {
			...
			if ( ! empty( $opts['do_seo'] ) ) {
				if ( ! class_exists( 'ReadyStudio_Module_SEO' ) ) {
					throw new Exception( 'ماژول SEO نصب نیست (Class Missing).' );
				}
				...
				// Save the data
				// This will also update the main post title inside Core Data
			...
			if ( ! empty( $opts['do_content'] ) ) {
				if ( ! class_exists( 'ReadyStudio_Module_Content' ) ) {
					throw new Exception( 'ماژول محتوا نصب نیست.' );
				}
			...
			// --- Alt-Only Generation (via Vision) ---
			...
			// Returns the EXACT error message to the log console instead of crashing
			wp_send_json_error( 'خطای سیستمی: ' . $e->getMessage() . ' (در خط ' . $e->getLine() . ')' );
			return;
		}
