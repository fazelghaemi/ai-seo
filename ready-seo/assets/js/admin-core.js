/*
 * Ready Studio SEO Engine - Admin Core JS (v12.9)
 *
 * v12.9: Added logic for the new Robots.txt Generator page.
 * v12.5: Made log messages in bulk generator clearer.
...
jQuery(document).ready(function ($) {
	...
	var $settingsPage = $('.rs-wrap.settings-page[dir="rtl"]:not(:has(#rs-gen-robots-btn))'); // Target settings page, but NOT robots page
		...
		$bulkStartBtn.click(function () {
			...
			function processNextPost() {
				...
				.always(function (xhr, status) {
					...
					// Only continue if the previous request didn't fail fatally (status 500)
	...
	// --- 3. Robots.txt Page Logic ---
	var $robotsPage = $('.rs-wrap.settings-page:has(#rs-gen-robots-btn)'); // Target Robots page
	if ($robotsPage.length) {
		
		var $aiRules = $('#ai-rules');
		var $userRules = $('#custom-rules');
		var $preview = $('#robots-preview');
		var sitemap = $preview.text().split('Sitemap: ')[1] || ''; // Grab sitemap from <pre>
